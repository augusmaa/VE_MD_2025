        ...
        if use_emotion: 
            ...
            if dist.get_rank() == 0:
                ...
                if avg_acc > best_acc:
                    ...
                    print(f"Saved best Emotion to {fname}")
...
def main(rank, world_size):
    ...
    parser.add_argument('--use_emotion', action='store_true',
                       help="to use emotion in the training (default:False)")  # we can decide to predict only pose also for test
    parser.add_argument('--use_mmd', action='store_true',
                       help="to use MMD loss to VE of Vanilla in the training (default:False)")       
    
    # Deocder skeleton
    parser.add_argument('--decoder_sktname',type=str, default='openpose',metavar='name',
                       help='Decoder to use: openpose; ..., (default:"openpose")')
    ...
    parser.add_argument('--datasets', type=str, nargs='+', default=['vgaf', 'gaf3'], 
                        help="List of datasets to use in training. Options: 'vgaf','gaf3', 'dfew', 'mer', 'engagenet', 'mersemi")
    ...
    parser.add_argument('--batch_dfew',type=int,default=10, metavar='batch',
                        help='batch size for training emotion (default: 10)')
    parser.add_argument('--batch_mersemi', type=int, default=10, metavar='batch',
                        help='batch size for training mersemi  (default: 10)')
    
    # Hyper parameters
    parser.add_argument('--LR_emotion', type=float, default=0.0000001, metavar='LR',
                        help='Learning rate for the optimizer (default:0.0000001)')
    parser.add_argument('--LR_mersemi', type=float, default=0.0000001, metavar='LR',
                        help='Learning rate for the optimizer (default:0.0000001)')
    parser.add_argument('--LR_coco', type=float, default=0.0000001, metavar='LR',
                        help='Learning rate for the optimizer (default:0.0000001)')
    parser.add_argument('--momentum', type=float, default=0.9,
                        help='momentum for the optimizer (default:0.9)')
    parser.add_argument('--beta_weight_pose',type=float,default=1.0, metavar='beta',
                        help='value for giving weight pose loss   (default:1.0)')
    parser.add_argument('--beta_weight_face',type=float,default=1.0, metavar='beta',
                        help='value for giving weight pose loss   (default:1.0)')
    ...
    dataset_configs = {
            ...
            'mersemi':   {'nb_frames': None,'num_class': None},  # no emotion head
            }
    ...
    nb_frames_video = {ds: dataset_configs[ds]['nb_frames'] for ds in args.datasets}    
    # device     
    
    if args.use_emotion:
        # pretrained vit path for video emotion
        if (args.datasets==['vgaf']):
            print("Dataset Emotion:", args.datasets)
    ...
    model = VE_MultiDecoder(device=device,
                            ...
                            decoder_sktname= args.decoder_sktname,
   ...
   # parameters 
    ...
    pose_loss = StageHeatmapLossMSE()

    #Loading DATA
    if  args.nb_frames_vgaf == 5:
        Dir_vgaf_train = os.path.join(str(args.dir_data_parent),'VGAF_Video_images_5/Train_heatmap_56_2')
        ...
        Dir_vgaf_train = os.path.join(str(args.dir_data_parent),'VGAF_Video_images_25/Train_heatmap_56')
    ...
    Dir_gaf3_train_heatmap = os.path.join(str(args.dir_data_parent),'GAF_3.0/Train_heatmap_56_2')
    ...
    Dir_dfew_train = os.path.join(str(args.dir_data_parent),'DFEW/Train_heatmap_56_2')
    ...
    dfew_train_csvfile = os.path.join(str(args.dir_data_parent),'DFEW/train_video_annotation_fa_kp_set_1.json') # set_2,set_3 ...set_5
    dfew_val_csvfile = os.path.join(str(args.dir_data_parent),'DFEW/test_label/set_1.csv') #set_2,set_3 ...set_5

    Dir_mer_train = os.path.join(str(args.dir_data_parent),'MER2023_10/Train_heatmap_56_2')
    ...
    mer_train_csvfile = os.path.join(str(args.dir_data_parent),'MER2023_10/train_video_annotation_all_vitpose_fa_limbs.json')
    mer_val_csvfile =  os.path.join(str(args.dir_data_parent),'MER2023_10/test1-label.csv') # 'test2-label.csv
    
    Dir_samsemo_train = os.path.join(str(args.dir_data_parent),'SamSemo/Train_heatmap_56_2')
    ...
    Dir_engage_train = os.path.join(str(args.dir_data_parent),'Engage_images_10/Train_heatmap_56_2')
    ...
    engage_train_csvfile = os.path.join(str(args.dir_data_parent),'Engage_images_10/train_video_annotation_all_vitpose_fa_limbs.json')
    engage_val_csvfile = os.path.join(str(args.dir_data_parent),'Engage_images_10/val_labels.txt')
    

    Dir_mersemi = os.path.join(str(args.dir_data_parent),'MER2023_5/Train_skeleton_fa_tensor')
    mersemi_train_csvfile = os.path.join(str(args.dir_data_parent),'MER2023_5/mersemi_video_annotation_all_vitpose_kp_fa.json')
    ...
    LoaderTrain = Train_DataLoaders(rank,
                                    ...
                                    batch_mersemi=args.batch_mersemi,  
                                    )
    ...
    # save model name 
    save_name_model = 'VE_MD_Heatmap_skt_decod_'+str(args.decoder_sktname)+'_'+str(args.encoder_name)+'_use_emotion_'+str(args.use_emotion)+'_use_mmd_'+str(args.use_mmd)+\
        '_use_pers_'+str(args.use_person_pose)+'_use_face_'+str(args.use_face_pose)+'_two_encoders_'+str(args.two_encoders)\
            +'_Add_Keyppoints_'+str(args.add_keypoints)+'_latent_dim_'+str(args.latent_dim)+'_'+str("_".join(args.datasets))+'_classif_proj_'+str(args.classif_projection)
    ...
    model = train_model(model,
                        ...
                        beta_weight_face = args.beta_weight_face,
