...
def get_loader_data_pairs(Loader, selected_datasets):
    available_datasets = {
        ...
        'mersemi': (Loader['mersemi'], 'mersemi'),
       }
...
def Train_epoch(model,
    ...
    for batch_data, ds in random_loader_iterator(get_loader_data_pairs(LoaderTrain, selected_datasets)):
        ...
        fwd = {'dataset': ds}
        # comment this beacause the vit branch directly
        ...
        # — emotion head
        if cfg['has_emotion']:# and  getattr(model, 'use_emotion', True):
        ...
        if cfg['has_person'] and  getattr(model, 'use_person', True):
            ...
            if (kp_p is not None):
                kp_gtp  = batch_data['limb_person'].to(device)
                l_p = pose_loss(kp_p,  kp_gtp)
        ...
        if cfg['has_face'] and getattr(model, 'use_face', True):
            ...
            if (kp_f is not None):
                kp_gtf = batch_data['limb_face'].to(device)
                l_f = pose_loss(kp_f,  kp_gtf)
                
                l_f   = beta_weight_face * l_f
